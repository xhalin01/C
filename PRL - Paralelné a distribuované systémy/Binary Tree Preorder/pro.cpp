...
int main(int argc, char* argv[])
{
    ...
    ///////////////////////////////////////////////////////SEND SUFFIX TO MASTER NODE
    if(myWeight==1){
        int myPreorder=size-val[myValIndex]+1;

        MPI_Send(&myId, size, MPI_INT, 0, TAG, MPI_COMM_WORLD);
        MPI_Send(&myPreorder, size, MPI_INT, 0, TAG, MPI_COMM_WORLD);
    }
    ...
    if(myId==0){
        ...
        {
            while(tempVal[next]==0 && next<=size){
                next++;
            }
            waitFrom[i]=next;
            next++;
        ...
        ///////////////////////////////////////////////RECIEVE PREORDERS FROM FORWARD NODES

        for (int i = 0; i < edgeNum; ++i)
        {
            MPI_Recv(&rvcId, size, MPI_INT, waitFrom[i], TAG, MPI_COMM_WORLD, &mpiStatus);
            MPI_Recv(&rcvPre, size, MPI_INT, waitFrom[i], TAG, MPI_COMM_WORLD, &mpiStatus);
            preorder[i][0]=rvcId;
            preorder[i][1]=rcvPre;
        }

        int preorderIndexes[edgeNum];
        next=0;
        for (int i = 0; i < edgeNum; ++i)
        {
            bool notFound=true;
            while(notFound){
                ...
                {
                    if(preorder[j][1]==next){
                        notFound=false;
                        preorderIndexes[i]=preorder[j][0];
                    }
                }
                next++;
            }
        ...
        char n;
        for (int i = 0; i < edgeNum; ++i){
            n=tempList[preorderIndexes[i]]->forward.to;
            printf("%c",n);
        }
        printf("\n");
    }
